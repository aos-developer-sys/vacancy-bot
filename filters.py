...
# Явное упоминание другого города/региона (не Москвы) - вакансия отбрасывается.
# Вакансии без указания города и remote-вакансии пропускаются (не отбрасываются).
#
# Короткие/частые названия ловятся только по границам слова с обеих сторон -
# иначе "Чита" совпадает внутри "Читать", "Тула" внутри "стула", "Сочи" внутри
# "сочинение" и т.п. Остальные матчатся с границей только слева, чтобы ловить
# и падежные формы вида "в Ижевске"/"в Новосибирске" (город + окончание).
# Ограничение: города на мягкий знак (Пермь, Тверь, Казань, Рязань) в падежных
# формах ("в Перми") всё равно не ловятся - основа меняется, не просто добавляется
# окончание. Если увидишь, что такие вакансии проскакивают - дай знать, дополним.
NON_MOSCOW_CITIES_STRICT = {
    'чита', 'тула', 'сочи', 'тамбов', 'уфа', 'омск', 'спб', 'орёл', 'орел',
}
NON_MOSCOW_CITIES = [
    'санкт-петербург', 'спб', 'питер', 'новосибирск', 'екатеринбург', 'казань',
    ...
    'новокузнецк', 'рязань', 'астрахань', 'пенза', 'липецк', 'киров',
    'чебоксары', 'тула', 'калининград', 'брянск', 'курск', 'иваново',
    'магнитогорск', 'тверь', 'ставрополь', 'белгород', 'сочи', 'сургут',
    'владимир', 'архангельск', 'чита', 'калуга', 'смоленск', 'волжский',
    'череповец', 'вологда', 'мурманск', 'якутск', 'стерлитамак', 'грозный',
    'тамбов', 'орёл', 'орел', 'нижний тагил', 'набережные челны', 'тайшет',
    'минск', 'алматы', 'астана', 'ташкент', 'бишкек', 'ереван', 'баку', 'тбилиси',
]

def _build_city_pattern(word: str) -> re.Pattern:
    if word in NON_MOSCOW_CITIES_STRICT:
        return re.compile(r'\b' + re.escape(word) + r'\b', re.IGNORECASE)
    return re.compile(r'\b' + re.escape(word), re.IGNORECASE)

NON_MOSCOW_PATTERNS = [_build_city_pattern(w) for w in NON_MOSCOW_CITIES]

def is_moscow_or_unspecified(text: str) -> bool:
    """Отбрасывает вакансии с явным упоминанием другого города. Если города нет
    в тексте вообще, либо указано remote/удалённо - вакансия пропускается."""
    return not any(pattern.search(text) for pattern in NON_MOSCOW_PATTERNS)
...
def is_vacancy_suitable(text: str) -> bool:
    ...
    # Проверка региона - только Москва (без города/remote пропускаем)
    if not is_moscow_or_unspecified(text):
        return False

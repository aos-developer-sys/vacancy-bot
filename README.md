...
- **API HH.ru для соискателей закрыт с 15.12.2025.** Официальный `api.hh.ru/vacancies` теперь требует подтверждённого аккаунта работодателя. Обошли это публичным RSS-экспортом поиска (`hh.ru/search/vacancy/rss`) + JSON-LD со страницы вакансии для полного текста описания — оба канала остаются открытыми для анонимного доступа.
...
- **Русская морфология ломает наивный substring-match.** Слово `продажи` не матчилось с `«сопровождения продаж»` из-за разных окончаний. Там, где это важно, список использует основы слов (`продаж` вместо `продажи`), а не точные словоформы.
- **Дедупликация без внешней БД.** `hh_seen.sqlite3` хранит ID уже просмотренных вакансий — каждый запуск обрабатывает только дельту, ничего не присылается повторно.
...
| `hh.ru/search/vacancy/rss` | Источник вакансий HH.ru | Официальный API для соискателей закрыт; RSS остаётся открытым и не требует OAuth |
| JSON-LD (`schema.org/JobPosting`) | Полный текст описания вакансии | Структурированные данные со страницы вакансии — не нужно парсить вёрстку |
| SQLite | Дедуп | Простое персистентное хранилище ID без внешней БД |
...
`.env` должен содержать `API_ID`, `API_HASH`, `PHONE` (данные Telegram-приложения с [my.telegram.org](https://my.telegram.org)) и `TARGET_CHANNEL`. Список отслеживаемых каналов и поисковый запрос HH.ru настраиваются в `config.py`.
